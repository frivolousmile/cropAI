    ...
    <div style={{
      ...
      backgroundImage: `
        linear-gradient(
          to bottom,
          rgba(5, 40, 15, 0.72) 0%,
          rgba(10, 60, 25, 0.60) 40%,
          rgba(5, 30, 10, 0.80) 100%
        ),
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ textAlign: 'center', padding: '2rem 0 2.5rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.8rem', fontWeight: 700, color: '#fff', textShadow: '0 2px 12px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ display: 'grid', gap: '1.5rem' }}>
            ...
            <div style={{ ...glassCard, padding: '2rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                ...
                <div style={{
                  ...
                  border: `1px solid ${sensors.timestamp ? 'rgba(16,185,129,0.5)' : 'rgba(239,68,68,0.5)'}`,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
                {[
                  { value: `${sensors.temperature || 0}°C`, label: 'Temperature', sub: 'DHT22 Sensor', color: '#fb923c', bg: 'rgba(251,146,60,0.15)' },
                  { value: `${sensors.humidity || 0}%`, label: 'Humidity', sub: 'DHT22 Sensor', color: '#38bdf8', bg: 'rgba(56,189,248,0.15)' },
                  { value: `${sensors.soil_moisture || 0}%`, label: 'Soil Moisture', sub: 'Soil Probe', color: '#a78bfa', bg: 'rgba(167,139,250,0.15)' },
                ].map((s, i) => (
                  <div key={i} style={{
                    background: s.bg,
                    ...
                    <div style={{ color: '#fff', fontWeight: 600, marginTop: '0.3rem' }}>{s.label}</div>
                    <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.82rem' }}>{s.sub}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ ...glassCard, padding: '2rem', borderTop: '3px solid #10b981' }}>
              ...
              {loading ? (
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: '2.8rem', fontWeight: 800, color: '#6ee7b7', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                    {latestPrediction.crop}
                  </div>
                  <div style={{ fontSize: '1.2rem', color: '#a7f3d0', margin: '0.75rem 0 0.5rem' }}>
                    Confidence: <strong>{latestPrediction.confidence}</strong>
                  </div>
                  <div style={{ fontSize: '0.95rem', color: 'rgba(255,255,255,0.65)' }}>
            ...
            <div style={{ ...glassCardDark, padding: '1.5rem', display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              <button onClick={testSensors} disabled={loading} style={{
                padding: '0.9rem 2rem',
                background: loading ? 'rgba(16,185,129,0.15)' : 'rgba(16,185,129,0.3)',
          ...
          <div style={{ ...glassCard, padding: '2.5rem', borderTop: '3px solid #3b82f6', maxWidth: '560px', margin: '0 auto' }}>
            ...
            <form onSubmit={handleManualPredict}>
              <div style={{ marginBottom: '1.5rem' }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, color: 'rgba(255,255,255,0.85)', fontSize: '0.95rem' }}>
                ...
                <select
                  ...
                  style={{ ...inputStyle }}
                >
                  <option value="" style={{ background: '#1a2e1a', color: '#fff' }}>Choose soil type...</option>
                  <option value="loamy" style={{ background: '#1a2e1a', color: '#fff' }}>🌾 Loamy Soil</option>
                  <option value="clay" style={{ background: '#1a2e1a', color: '#fff' }}>🏺 Clay Soil</option>
                  <option value="sandy" style={{ background: '#1a2e1a', color: '#fff' }}>🏖️ Sandy Soil</option>
                  <option value="black" style={{ background: '#1a2e1a', color: '#fff' }}>⚫ Black Soil</option>
                  <option value="red" style={{ background: '#1a2e1a', color: '#fff' }}>🔴 Red Soil</option>
                  <option value="alluvial" style={{ background: '#1a2e1a', color: '#fff' }}>🏔️ Alluvial Soil</option>
                </select>
              </div>
              ...
              <button type="submit" disabled={loading || !soilType} style={{
                ...
                background: loading || !soilType ? 'rgba(59,130,246,0.15)' : 'rgba(59,130,246,0.4)',
                border: '1px solid rgba(59,130,246,0.6)',
                ...
                <thead>
                  <tr style={{ background: 'rgba(255,255,255,0.06)' }}>
                    {['Crop', 'Confidence', 'Location', 'Date'].map(h => (
                      <th key={h} style={{ padding: '0.9rem 1.25rem', textAlign: 'left', fontWeight: 600, color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {history.map((pred, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <td style={{ padding: '1rem 1.25rem', color: 'rgba(255,255,255,0.6)' }}>{pred[0]}</td>
      ...
      <div style={{
        ...
        textAlign: 'center', padding: '1.5rem', marginTop: '4rem', fontSize: '0.85rem',
